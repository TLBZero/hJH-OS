...
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    uint8 *v_buf = kmalloc(cmd_len + tx_len);
    uint64 i;
    for(i = 0; i < cmd_len; i++)
        v_buf[i] = cmd_buff[i];
    for(i = 0; i < tx_len; i++)
                                 ...
                                 uint64 cmd_len, void *rx_buff, uint64 rx_len)
{
    ...
    // printf("[spi_receive_data_normal_dma]debug 7\n");
    spi_handle->ser = 0x00;
    spi_handle->ssienr = 0x00;
}
                              ...
                              const void *tx_buff, uint64 tx_len, spi_transfer_width_t spi_transfer_width)
{
    ...
    {
        case SPI_TRANS_SHORT:
            buf = (uint32 *)kmalloc((tx_len) * sizeof(uint32));
            for(i = 0; i < tx_len; i++)
                                   ...
                                   uint64 cmd_len, uint8 *rx_buff, uint64 rx_len)
{
    ...
    switch(frame_width)
    {
        case SPI_TRANS_INT:
            ...
            break;
        case SPI_TRANS_SHORT:
            write_cmd = kmalloc((cmd_len + rx_len) / 2 * sizeof(uint32));
            ...
            break;
        default:
            write_cmd = kmalloc((cmd_len + rx_len) * sizeof(uint32));
            for(i = 0; i < cmd_len; i++)
    ...
    printf("[spi_receive_data_standard_dma]before dma\n");
    ...
    printf("[spi_receive_data_standard_dma]after dma\n");
    switch(frame_width)
    {
        case SPI_TRANS_INT:
                                ...
                                const uint8 *cmd_buff, uint64 cmd_len, const uint8 *tx_buff, uint64 tx_len)
{
    ...
    switch(frame_width)
    {
        case SPI_TRANS_INT:
            ...
            break;
        case SPI_TRANS_SHORT:
            buf = kmalloc((cmd_len + tx_len) / 2 * sizeof(uint32));
            for(i = 0; i < cmd_len / 2; i++)
            ...
            break;
        default:
            buf = kmalloc((cmd_len + tx_len) * sizeof(uint32));
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len; i++)
