...
int32 dmac_check_channel_busy(dmac_channel_number_t channel_num)
{
    int32 ret = 0;
...
int32 dmac_set_list_master_select(dmac_channel_number_t channel_num,
                           ...
                           uint32 blockSize)
{
...
#if FIX_CACHE
    uint8 *src_io = (uint8 *)src;
    uint8 *dest_io = (uint8 *)dest;
    if(is_memory_cache((uintptr_t)src))
    {
        if(src_inc == DMAC_ADDR_NOCHANGE)
        {
            src_io = (uint8 *)iomem_malloc(1<<dmac_trans_width);
            memcpy(src_io, src, 1<<dmac_trans_width);
        }
        else
        {
            src_io = (uint8 *)iomem_malloc(blockSize * (1<<dmac_trans_width));
    ...
    if(is_memory_cache((uintptr_t)dest))
    {
        ...
        {
            dest_io = (uint8 *)iomem_malloc(1<<dmac_trans_width);
            dmac_context[channel_num].buf_len = 1<<dmac_trans_width;
        }
        else
        {
            dest_io = (uint8 *)iomem_malloc(blockSize * (1<<dmac_trans_width));
...
#if FIX_CACHE
    dmac->channel[channel_num].sar = (uint64)src_io;
    dmac->channel[channel_num].dar = (uint64)dest_io;
#else
...
void dmac_set_address(dmac_channel_number_t channel_num, uint64 src_addr,
                      uint64 dst_addr)
{
                                     ...
                                     uint64 *addr)
{
    ...
    /* Cast pointer to uint64 */
    llp_u.llp.loc = (uint64)addr;
...
void dmac_link_list_item(dmac_channel_number_t channel_num,
                         uint8 LLI_row_num, int8 LLI_last_row,
                         ...
                         dmac_channel_config_t *cfg_param)
{
    ...
    if(LLI_last_row != LAST_ROW)
        llp_u.llp.loc = ((uint64)&lli_item[LLI_row_num + 1]) >> 6;
    else
        llp_u.llp.loc = 0;
...
void dmac_update_shandow_register(dmac_channel_number_t channel_num,
                                  int8 last_block, dmac_channel_config_t *cfg_param)
{
    dmac_ch_ctl_u_t ctl_u;
...
void __attribute__((weak, alias("dmac_irq_register"))) dmac_set_irq(dmac_channel_number_t channel_num, plic_irq_callback_t dmac_callback, void *ctx, uint32 priority);
